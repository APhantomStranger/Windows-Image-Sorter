...
from PySide6.QtGui import (
    ...
    QPixmap, QImage, QPainter, QPainterPath, QDrag,
)
...
class MainWindow(QMainWindow):
    ...
    def _drop_group_up(self, gid):
        ...
        if g and g.parent:
            ...
            self.log(f"Moved group \u201c{g.name}\u201d up a level")
    ...
    def _on_batch_done(self, summary):
        ...
        if summary["mode"] == "undo":
            ...
            if summary["errors"]:
                parts.append(f"{len(summary['errors'])} failed")
            self.statusBar().showMessage(" \u00b7 ".join(parts), 6000)
            if summary["undo"]:
                self.undo_stack.append(summary["undo"])
            if summary["errors"]:
                detail = "\n".join(f"- {n}: {e}" for n, e in summary["errors"][:12])
                more = "" if len(summary["errors"]) <= 12 else \
                    f"\n...and {len(summary['errors']) - 12} more"
                QMessageBox.warning(
                    self, "Some files could not be filed", detail + more)
